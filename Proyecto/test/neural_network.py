...
class Layer:
    ...
    def mutate(self, verbose=False):
        r = random.random()
        if r < 0.5:
            k = random.randint(0, len(self.b)-1)
            self.b[k] = np.random.normal()
            if verbose:
                print("Mutated bias", k)
        else:
            row = random.randint(0, len(self.W)-1)
            col = random.randint(0, len(self.W[0])-1)
            self.W[row,col] = np.random.normal()
            if verbose:
                print("Mutated weights ({},{})".format(row, col))
    ...
    def decode(self, sequence):
        ...
        W = sequence[:W_size].reshape(self.W.shape)
        b = sequence[W_size:]
...
class NeuralNet:
    ...
    def mutate(self, verbose=False):
        ...
        if verbose:
            print("Mutated layer", r)
        self.layers[r].mutate(verbose=verbose)
